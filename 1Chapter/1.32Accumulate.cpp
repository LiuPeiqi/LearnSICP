...
namespace LPQ {
    ...
    double Accumulate(COMBINER combiner, double null_value,
        UNIQUE_OP term, double a, UNIQUE_OP next, double b)
    {
        if (a > b) {
            return null_value;
        }
    ...
    double AccumulateIter(COMBINER combiner, double null_value,
        UNIQUE_OP term, double a, UNIQUE_OP next, double b)
    {
    ...
    static double Sum(UNIQUE_OP term, double a, UNIQUE_OP next, double b) {
        return AccumulateIter([](auto a, auto b) {return a + b; }, 0,
            term, a, next, b);
    }
    static double Product(UNIQUE_OP term, double a, UNIQUE_OP next, double b) {
        return AccumulateIter([](auto a, auto b) {return a * b; }, 1,
            term, a, next, b);
    }

    double FilteredAccumulate(FILTER filter, COMBINER combiner, double null_value,
        UNIQUE_OP term, double start, UNIQUE_OP next, double end)
    {
        double result = null_value;
    ...
    {
        return FilteredAccumulate([](double a) {return IsPrime(static_cast<long long>(a)); },
            [](auto a, auto b) {return a + b; }, 0,
            [](auto a) {return a; }, a, [](auto a) {return a + 1; }, b);
    }
    int PrimeSum(istream& in, ostream& out) 
    {
        double start, end;
        in >> start >> end;
        out << PrimeSum(start, end);
        return 0;
    }
    ...
    double CoPrimeProduct(double a, double b)
    {
        ...
        return FilteredAccumulate(filter, [](auto a, auto b) {return a*b; }, 1,
            [](auto a) {return a; }, a, [](auto a) {return a + 1; }, b);
    }
    int CoPrimeProduct(istream& in, ostream& out)
    {
        double start, end;
        in >> start >> end;
        out << CoPrimeProduct(start, end);
        return 0;
    }
}